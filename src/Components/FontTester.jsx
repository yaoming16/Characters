...
function FontTester({ font }) {
  ...
  return (
    <div className="mt-4 p-4 border border-gray-300 rounded">
      ...
      <p><strong>Current font-family:</strong> {font}</p>
      ...
      <div className="mt-2">
        ...
        <ul className="list-disc ml-4">
          {fontList.map((singleFont, index) => (
            <li key={index}>
              {singleFont}: {checkFontLoaded(singleFont) ? "✅ Available" : "❌ Not available"}
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className="mt-4">
        <strong>Font comparison with test character &apos;{testCharacter}&apos;:</strong>
        <div className="grid grid-cols-1 gap-2 mt-2">
          {['serif', '"Noto Serif SC"', '"FangSong"', '"KaiTi"', '"SimSun"'].map((testFont, index) => (
